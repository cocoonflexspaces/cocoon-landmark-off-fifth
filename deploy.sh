...
pages_live() {
  gh api "/repos/${GH_USER}/${REPO_NAME}/pages" >/dev/null 2>&1
}

VISIBILITY="$(gh repo view "${GH_USER}/${REPO_NAME}" --json visibility --jq .visibility)"
if [ "$VISIBILITY" = "PRIVATE" ]; then
  echo "  Repo is private — trying to enable Pages directly (works on GitHub Pro/Team)…"
  enable_pages || true
  if pages_live; then
    echo "  ✓ Pages enabled on the private repo. (Note: the published site itself is publicly viewable by anyone with the URL.)"
  else
    read -rp "  Your plan doesn't allow Pages on private repos. Make this repo public? [y/N] " MK
    if [[ "$MK" =~ ^[Yy]$ ]]; then
